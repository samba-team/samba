# Checks for programs.
# Unique-to-Samba variables we'll be playing with.
...
AC_HEADER_SYS_WAIT
check_headers((
	'arpa/inet.h', 'sys/select.h', 'fcntl.h', 'sys/fcntl.h', 'sys/time.h',
	'utime.h', 'grp.h', 'sys/id.h', 'limits.h', 'memory.h', 'net/if.h',
	'compat.h', 'sys/param.h', 'ctype.h', 'sys/wait.h', 'sys/resource.h',
	'sys/ioctl.h', 'sys/ipc.h', 'sys/mode.h', 'sys/mman.h', 'sys/filio.h',
	'sys/priv.h', 'sys/shm.h', 'string.h', 'strings.h', 'stdlib.h', 'sys/socket.h'));
check_headers(sys/mount.h sys/vfs.h sys/fs/s5param.h sys/filsys.h termios.h termio.h)
check_headers(sys/termio.h sys/statfs.h sys/dustat.h sys/statvfs.h stdarg.h sys/sockio.h)
check_headers(security/pam_modules.h security/_pam_macros.h dlfcn.h)
check_headers(sys/syslog.h syslog.h)
check_headers(stdint.h)
...
check_headers('shadow.h', 'netinet/ip.h', netinet/tcp.h netinet/in_systm.h netinet/in_ip.h)
check_headers(nss.h nss_common.h ns_api.h sys/security.h security/pam_appl.h security/pam_modules.h)
check_headers(stropts.h)
check_headers(sys/capability.h syscall.h sys/syscall.h)
check_headers(sys/acl.h)
...
if test x"$ac_cv_func_connect" = x"no"; then
    case "$LIBS" in
    *-lnsl*) ;;
    *) check_lib(nsl_s, printf) ;;
    esac
    case "$LIBS" in
    *-lnsl*) ;;
    *) check_lib(nsl, printf) ;;
    esac
    case "$LIBS" in
    *-lsocket*) ;;
    *) check_lib(socket, connect) ;;
    esac
    case "$LIBS" in
    *-linet*) ;;
    *) check_lib(inet, connect) ;;
    esac
    # We can't just call check_funcs(connect) here, because the value
    # has been cached.
...
check_funcs(yp_get_default_domain)
...
check_funcs(dlopen dlclose dlsym dlerror waitpid getcwd strdup strndup strnlen strtoul strtoull strtouq strerror chown fchown chmod fchmod chroot link mknod mknod64)
check_funcs(fstat strchr utime utimes getrlimit fsync bzero memset strlcpy strlcat setpgid)
check_funcs(memmove vsnprintf snprintf asprintf vasprintf setsid glob strpbrk pipe crypt16 getauthuid)
check_funcs(strftime sigprocmask sigblock sigaction sigset innetgr setnetgrent getnetgrent endnetgrent)
check_funcs(initgroups select rdchk getgrnam getgrent pathconf realpath)
check_funcs(setpriv setgidx setuidx setgroups sysconf mktime rename ftruncate)
check_funcs(setluid getpwanam setlinebuf)
check_funcs(srandom random srand rand setenv usleep strcasecmp fcvt fcvtl symlink readlink)
check_funcs(syslog vsyslog getgrouplist timegm backtrace)
# setbuffer, shmget, shm_open are needed for smbtorture
check_funcs(setbuffer shmget shm_open)

# syscall() is needed for smbwrapper.
check_funcs(syscall)

check_funcs(getdents)
check_funcs(pread pwrite)
...
#
# Don't check for 64 bit fcntl locking if we know that the
# glibc2.1 broken check has succeeded.
# 
...
# statvfs function.  This check is more than just check_funcs(statvfs)
...
if test $space = no; then
# AIX
  AC_MSG_CHECKING([for two-argument statfs with statfs.bsize #
member (AIX, 4.3BSD)])
...
if test $space = no; then
# 4.4BSD and NetBSD
  AC_MSG_CHECKING([for two-argument statfs with statfs.fsize #
...
check_headers(sys/attributes.h attr/xattr.h sys/xattr.h)
AC_SEARCH_LIBS(getxattr, [attr])
check_funcs(getxattr lgetxattr fgetxattr listxattr llistxattr)
check_funcs(flistxattr removexattr lremovexattr fremovexattr)
check_funcs(setxattr lsetxattr fsetxattr)
check_funcs(attr_get attr_list attr_set attr_remove)
check_funcs(attr_getf attr_listf attr_setf attr_removef)
...
[ case "$withval" in
  yes)
	...
	*freebsd5*)
		...
		;;
	*linux*)
		check_lib(attr,getxattr,[ACL_LIBS="$ACL_LIBS -lattr"])
       		check_lib(acl,acl_get_file,[ACL_LIBS="$ACL_LIBS -lacl"])
