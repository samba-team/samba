...
BOOL lookup_name(const char *domain, const char *name, DOM_SID *psid, enum samr_SidType *name_type)
{
	fstring sid;
...
BOOL lookup_sid(const DOM_SID *sid, fstring dom_name, fstring name, enum samr_SidType *name_type)
{
	if (!name_type)
		return False;
