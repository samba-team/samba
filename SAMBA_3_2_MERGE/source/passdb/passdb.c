...
BOOL local_lookup_sid(const DOM_SID *sid, char *name, enum samr_SidType *psid_name_use)
{
	uint32 rid;
...
BOOL local_lookup_name(const char *c_user, DOM_SID *psid, enum samr_SidType *psid_name_use)
{
	DOM_SID local_sid;
	DOM_SID sid;
	fstring user;
...
BOOL local_sid_to_uid(uid_t *puid, const DOM_SID *psid, enum samr_SidType *name_type)
{
	SAM_ACCOUNT *sampw = NULL;	
	struct passwd *unix_pw;
...
BOOL local_sid_to_gid(gid_t *pgid, const DOM_SID *psid, enum samr_SidType *name_type)
{
	uint32 rid;
	GROUP_MAP group;
	BOOL ret;
