...
static int generic_job_delete(int snum, struct printjob *pjob)
{
	...
	if ( !(mem_ctx = talloc_init( "generic_job_delete" )) ) {
		DEBUG(0,("generic_job_delete: talloc_init() failed!\n"));
		return 1;
	}

	/* need to delete the spooled entry */
	slprintf(jobstr, sizeof(jobstr)-1, "%d", pjob->sysjob);
	ret_val = print_run_command(
		   snum, 
		   lp_lprmcommand(snum), NULL,
		   "%j", jobstr,
		   "%T", http_timestring(mem_ctx, pjob->starttime),
		   NULL);
