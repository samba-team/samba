...
int net_rap_server(int argc, const char **argv)
{
	...
	ret = cli_NetServerEnum(cli, /*cli->server_domainFIXME*/NULL, SV_TYPE_ALL, 
...
int net_rap_domain(int argc, const char **argv)
{
	...
	ret = cli_NetServerEnum(cli, /*FIXMEcli->server_domain*/NULL, SV_TYPE_DOMAIN_ENUM,
				display_server_func,NULL);	
	cli_shutdown(cli);
	return ret;	
}
...
static int rap_printq_info(int argc, const char **argv)
{
	...
	d_printf(PRINTQ_ENUM_DISPLAY, /*FIXMEcli->desthost*/NULL); /* list header */
