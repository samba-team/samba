...
static int print_sam_info (SAM_ACCOUNT *sam_pwent, BOOL verbosity, BOOL smbpwdstyle)
{
	...
	if (verbosity) {
		...
		if ( !(mem_ctx = talloc_init( "print_sam_info" )) ) {
			printf("ERROR! print_sam_info: talloc_init() failed!\n");
			return -1;
		}
		tmp = pdb_get_logon_time(sam_pwent);
		printf ("Logon time:           %s\n", tmp ? http_timestring(mem_ctx, tmp) : "0");
...
static int print_trustpw_info(TALLOC_CTX *mem_ctx, SAM_TRUST_PASSWD *trust, BOOL verbose)
{
	...
	if (verbose) {
		...
		printf("Last modified         %s\n", trust->private.mod_time ? http_timestring(mem_ctx, trust->private.mod_time) : "0");
