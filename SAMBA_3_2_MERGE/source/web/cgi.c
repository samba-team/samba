...
static void cgi_download(char *file)
{
	...
	mem_ctx = talloc_init( "cgi_download" );
	if ( !mem_ctx ) {
		/* what can we do here to let the client know what happened? */
		close(fd);
		exit(1);
	}
	printf("Expires: %s\r\n", http_timestring(mem_ctx, time(NULL)+EXPIRY_TIME));
...
void cgi_setup(const char *rootdir, int auth_required)
{
	...
	printf("HTTP/1.0 200 OK\r\nConnection: close\r\n");
        mem_ctx = talloc_init( "cgi_setup" );
        if ( !mem_ctx ) {
                /* what can we do here to let the client know what happened? */
                return;
        }

	printf("Date: %s\r\n", http_timestring(mem_ctx, time(NULL)));
