...
struct sys_grent * getgrent_list(grent_filter_func f, void *data)
{
	...
	while (grp != NULL) {
		...
		if ( (f != NULL) && (!f(grp, data)) ) {
			/* Hmm. This routine needs reworking. A goto for this
			   purpose is not what you want... */
			goto next;
		}
