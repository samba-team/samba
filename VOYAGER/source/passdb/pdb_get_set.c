...
/* Ok, this is probably too inefficient, but as an example let's try it this
 * way */
...
uid_t pdb_get_uid(const SAM_ACCOUNT *sampass)
{
	...
	pwd = getpwnam(sampass->private.username);
	if (pwd == NULL) {
		DEBUG(0, ("getpwnam for SAM_ACCOUNT [%s] failed\n",
			  sampass->private.username));
		smb_panic("exiting\n");
	}
