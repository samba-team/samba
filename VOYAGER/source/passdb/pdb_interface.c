...
static NTSTATUS context_get_aliasname(struct pdb_context *context,
				      TALLOC_CTX *mem_ctx,
				      const DOM_SID *sid, char **name)
{
	...
	return context->pdb_methods->get_aliasname(context->pdb_methods,
						   mem_ctx, sid, name);
}
...
static NTSTATUS make_pdb_context(struct pdb_context **context) 
{
	...
	*context = talloc(mem_ctx, sizeof(**context));
	...
	(*context)->pdb_get_aliasname = context_get_aliasname;
...
BOOL pdb_get_aliasname(TALLOC_CTX *mem_ctx, const DOM_SID *sid, char **name)
{
	struct pdb_context *pdb_context = pdb_get_static_context(False);
	...
	return NT_STATUS_IS_OK(pdb_context->pdb_get_aliasname(pdb_context,
							      mem_ctx, sid,
							      name));
}
...
NTSTATUS make_pdb_methods(TALLOC_CTX *mem_ctx, PDB_METHODS **methods) 
{
	*methods = talloc(mem_ctx, sizeof(struct pdb_methods));
	...
	(*methods)->get_aliasname = pdb_default_get_aliasname;
