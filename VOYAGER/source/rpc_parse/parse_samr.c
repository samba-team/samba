			     ...
			     DOM_SID *domain_sid)
{
	...
	for (i = 0; i < num_entries ; i++) {
		...
		init_unistr2(&sam->str[i].uni_acct_name, pwd->name,
			     UNI_FLAGS_NONE);
		init_unistr2(&sam->str[i].uni_full_name, pwd->full_name,
			     UNI_FLAGS_NONE);
		init_unistr2(&sam->str[i].uni_acct_desc, pwd->desc,
			     UNI_FLAGS_NONE);
			     ...
			     DOM_SID *domain_sid )
{
	...
	for (i = 0; i < num_entries; i++) {
		...
		init_unistr2(&sam->str[i].uni_srv_name, pwd->name,
			     UNI_FLAGS_NONE);
		init_unistr2(&sam->str[i].uni_srv_desc, pwd->desc,
			     UNI_FLAGS_NONE);
			     ...
			     struct user_display_info *disp_user_info)
{
	...
	for (i = 0; i < num_entries; i++) {
		...
		DEBUG(11, ("init_sam_dispinfo_4: entry: %d\n",i));

		len_sam_name = strlen(pwd->name);
	  
		init_sam_entry4(&sam->sam[i], start_idx + i + 1, len_sam_name);

		init_string2(&sam->str[i].acct_name, pwd->name,
