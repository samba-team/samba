...
static NTSTATUS load_group_domain_entries(struct samr_info *info,
					  const DOM_SID *sid)
{
	...
	if (!sid_equal(sid, get_global_sam_sid())) {
		/* For now we only have domain groups in our global
		 * domain. BUILTIN might have to follow later. */
		info->disp_info.num_group_account=0;
	...
	for (gent = glist; gent != NULL; gent = gent->next)
		info->disp_info.num_group_account += 1;

	grp_array=(DOMAIN_GRP *)talloc(mem_ctx,
				       info->disp_info.num_group_account*
				       sizeof(DOMAIN_GRP));

	if (group_entries!=0 && grp_array==NULL) {
		DEBUG(1, ("load_group_domain_entries: talloc() failed for "
			  "grp_array!\n"));
		return NT_STATUS_NO_MEMORY;
	}
	...
	for (gent = glist; gent != NULL; gent = gent->next) {
		DOM_SID grp_sid;
		struct acct_info acct_info;
		gid_to_sid(&grp_sid, gent->gr_gid);

		if (pdb_get_dom_grp_info(&grp_sid, &acct_info)) {
			fstrcpy(grp_array[i].name, acct_info.acct_name);
			fstrcpy(grp_array[i].comment, acct_info.acct_desc);
		} else {
			fstrcpy(grp_array[i].name, gent->gr_name);
			fstrcpy(grp_array[i].comment, "");
		}
...
static void make_group_sam_entry_list(TALLOC_CTX *ctx, SAM_ENTRY **sam_pp, UNISTR2 **uni_name_pp,
                uint32 start_idx, uint32 num_sam_entries, DOMAIN_GRP *grp)
{
	uint32 i;
	SAM_ENTRY *sam;
	UNISTR2 *uni_name;
	...
	*uni_name_pp = NULL;
	...
	for (i = 0; i < num_sam_entries; i++) {
		 ...
		 */
		init_unistr2(&uni_name[i], grp[i+start_idx].name,
			     UNI_STR_TERMINATE);
		init_sam_entry(&sam[i], &uni_name[i], grp[i+start_idx].rid);
	}
...
NTSTATUS _samr_enum_dom_groups(pipes_struct *p, SAMR_Q_ENUM_DOM_GROUPS *q_u, SAMR_R_ENUM_DOM_GROUPS *r_u)
{
	...
	max_entries = info->disp_info.num_group_account - q_u->start_idx;

	if (max_entries > q_u->max_size) {
		max_entries = q_u->max_size;
		r_u->status = STATUS_MORE_ENTRIES;
	}

	make_group_sam_entry_list(p->mem_ctx, &r_u->sam, &r_u->uni_grp_name,
				  q_u->start_idx, max_entries,
				  info->disp_info.disp_group_info);

	init_samr_r_enum_dom_groups(r_u, q_u->start_idx + max_entries,
				    max_entries);
...
NTSTATUS _samr_enum_dom_aliases(pipes_struct *p, SAMR_Q_ENUM_DOM_ALIASES *q_u, SAMR_R_ENUM_DOM_ALIASES *r_u)
{
	...
	make_group_sam_entry_list(p->mem_ctx, &r_u->sam, &r_u->uni_grp_name, 0, num_entries, grp);
