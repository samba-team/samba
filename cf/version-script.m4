...
EOF
  ...
  if AC_TRY_COMMAND([$SHELL libtool --mode compile ${CC-cc} -c $CFLAGS conftest.c])  && 
     AC_TRY_COMMAND([$SHELL libtool --mode link ${CC-cc} $CFLAGS $LDFLAGS -o libconftestlib.la -Wl,--version-script,conftest.map conftest.lo]);
  then
