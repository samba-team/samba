...
while(@bins) {
  ...
  if (index($nextfile,'$')) {
    if ($filename eq "smbpasswd") {
      ...
      print IDB "f 4755 root sys usr/samba/bin/$filename source/$nextfile samba.sw.base nostrip exitop(/usr/samba/scripts/startswat.sh) removeop(/usr/samba/scripts/removeswat.sh)\n";
    }
