...
def debug_lineno(*args):
    ...
    print((" %s:" "\033[01;33m"
                          "%s " "\033[00m" % (tb[0][2], tb[0][1])), end=' ', file=sys.stderr)
    for a in args:
    ...
    def play(self, conversation, context):
        ...
        try:
            ...
            print("Conversation(%s) Missing handler %s" % \
                (conversation.conversation_id, fn_name), file=sys.stderr)
            return
...
class Conversation(object):
    ...
    def replay_in_fork_with_delay(self, start, context=None, account=None):
        ...
        if gap > 0.15 and False:
            print("sleeping for %f in main process" % (gap - 0.1), file=sys.stderr)
            time.sleep(gap - 0.1)
            now = time.time() - start
            gap = t - now
        ...
        try:
            ...
            print(("EXCEPTION in child PID %d, conversation %s" % (pid, self)), file=sys.stderr)
...
def generate_users_and_groups(ldb, instance_id, password,
    ...
    if (groups_added > 0 and users_added == 0 and
       number_of_groups != groups_added):
        print("Warning: the added groups will contain no members", file=sys.stderr)
