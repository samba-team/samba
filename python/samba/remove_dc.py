    ...
    for zone in zones:
        ...
        for record in records:
            ...
            if len(values) != orig_num_values:
                logger.info("updating %s keeping %d values, removing %s values" % (record.dn, len(values), orig_num_values - len(values)))
...
def offline_remove_ntds_dc(samdb,
    ...
    if seize_stale_fsmo:
        ...
        for role in stale_fsmo_roles:
            ...
            logger.warning("Seizing FSMO role on: %s (now owned by %s)" % (role.dn, my_serviceName))
            samdb.modify(m)
...
def remove_dc(samdb, logger, dc_name):
    ...
    try:
        ...
        try:
            ...
            raise demoteException("Failure checking if %s is an server object in %s: "
