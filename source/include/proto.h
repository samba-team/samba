...
uint32 spoolss_getprinter(const POLICY_HND *hnd, uint32 level,
                             NEW_BUFFER *buffer, uint32 offered,
                             uint32 *needed);
uint32 spoolss_setprinter(const POLICY_HND *hnd, uint32 level,
			  NEW_BUFFER *buffer, SEC_DESC_BUF *secdesc_ctr, 
			  DEVMODE_CTR *devmode_ctr, uint32 command);
...
BOOL msrpc_spoolss_setprinter( const char* printer_name, const uint32 level,
                const char* station, const char* user_name,
                PRINTER_INFO_CTR ctr);
...
BOOL make_spoolss_q_getprinter(SPOOL_Q_GETPRINTER *q_u, const POLICY_HND *hnd, uint32 level, 
				NEW_BUFFER *buffer, uint32 offered);
BOOL make_spoolss_q_setprinter(SPOOL_Q_SETPRINTER *q_u, const POLICY_HND *hnd, 
			       uint32 level, SEC_DESC_BUF *secdesc_ctr,
			       DEVMODE_CTR *devmode_ctr, uint32 command);
...
uint32 cmd_spoolss_setprinter(struct client_info *info, int argc, char *argv[]);
