                         		...
                         		struct in_addr **return_iplist, int *return_count)
{
  ...
  BOOL is_address = is_ip_address(name);
  BOOL result = False;
  ...
  *return_count = 0;
  ...
  while (next_token(&ptr, tok, LIST_SEP, sizeof(tok))) {
	  if((strequal(tok, "host") || strequal(tok, "hosts"))) {
		  ...
		  DEBUG(0,("internal_resolve_name: unknown name switch type %s\n", tok));
	  }
  }

  /* This code only called when all the resolve_* functions above return
     False. */
...
BOOL lookup_pdc_name(const char *srcname, const char *domain, struct in_addr *pdc_ip, char *ret_name)
{
  ...
  DEBUG(10, ("lookup_pdc_name: looking up pdc name for %s in domain %s\n",
	     inet_ntoa(*pdc_ip), domain));
...
NT GETDC call, UNICODE, NT domain SID and uncle tom cobbley and all...
	...
	bufp = ascii_to_unibuf(bufp, srcname, sizeof(buffer) - (bufp - buffer) - 1);
	SIVAL(bufp,0,1);
	SSVAL(bufp,4,0xFFFF); 
	SSVAL(bufp,6,0xFFFF); 
	bufp += 8;
