...
static NTSTATUS check_negative_conn_cache( const char *domain, const char *server )
{
	...
	for (fcc = failed_connection_cache; fcc; fcc = fcc->next) {
		...
		if ( !strequal_unix(domain, fcc->domain_name) || !strequal_unix(server, fcc->controller) )
...
void add_failed_connection_entry(const char *domain, const char *server, NTSTATUS result) 
{
	...
	for (fcc = failed_connection_cache; fcc; fcc = fcc->next) {
		if ( strequal_unix(fcc->domain_name, domain) && strequal_unix(fcc->controller, server) ) 
		{
