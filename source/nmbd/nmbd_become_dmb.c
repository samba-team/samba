...
in workgroup %s on subnet %s\n",
       global_myname_unix(), work->work_group, subrec->subnet_name));
    return;
  }
...
in workgroup %s on subnet %s\n", 
       global_myname_unix(), registered_name->name, subrec->subnet_name));
    work->dom_state = DOMAIN_NONE;
    return;
  }
  ...
  if( DEBUGLVL( 0 ) )
    {
    dbgtext( "*****\n\nSamba server %s ", global_myname_unix() );
  ...
  if( subrec == unicast_subnet )
  {
    ...
    make_nmb_name(&nmbname, global_myname_unix(), 0x20);
