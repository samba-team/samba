...
void register_my_workgroup_one_subnet(struct subnet_record *subrec)
{
	...
	if((work = create_workgroup_on_subnet(subrec, lp_workgroup_unix(), 
					      PERMANENT_TTL)) == NULL) {
...
Exiting.\n", lp_workgroup_unix(), subrec->subnet_name));
	...
	for (i=0; my_netbios_names_unix(i); i++) {
		register_name(subrec, my_netbios_names_unix(i),0x20,samba_nb_type,
			      NULL,
			      my_name_register_failed, NULL);
		register_name(subrec, my_netbios_names_unix(i),0x03,samba_nb_type,
			      NULL,
			      my_name_register_failed, NULL);
		register_name(subrec, my_netbios_names_unix(i),0x00,samba_nb_type,
			      NULL,
			      my_name_register_failed, NULL);
	}
...
BOOL register_my_workgroup_and_names(void)
{
  ...
  for (i=0; my_netbios_names_unix(i); i++)
  {
    for(subrec = FIRST_SUBNET; subrec; subrec = NEXT_SUBNET_EXCLUDING_UNICAST(subrec))
    {
      ...
      make_nmb_name(&nmbname, my_netbios_names_unix(i),0x20);
      insert_refresh_name_into_unicast(subrec, &nmbname, samba_nb_type);

      make_nmb_name(&nmbname, my_netbios_names_unix(i),0x3);
      insert_refresh_name_into_unicast(subrec, &nmbname, samba_nb_type);

      make_nmb_name(&nmbname, my_netbios_names_unix(i),0x0);
      insert_refresh_name_into_unicast(subrec, &nmbname, samba_nb_type);
    }
  }
  ...
  for(subrec = FIRST_SUBNET; subrec; subrec = NEXT_SUBNET_EXCLUDING_UNICAST(subrec))
  {
    ...
    make_nmb_name(&nmbname, lp_workgroup_unix(), 0x0);
    insert_refresh_name_into_unicast(subrec, &nmbname, samba_nb_type|NB_GROUP);

    make_nmb_name(&nmbname, lp_workgroup_unix(), 0x1e);
    insert_refresh_name_into_unicast(subrec, &nmbname, samba_nb_type|NB_GROUP);
  }
