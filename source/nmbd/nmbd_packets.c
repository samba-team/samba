...
static uint16 generate_name_trn_id(void)
{
	...
	{
		name_trn_id =
			((unsigned)time(NULL) % (unsigned)0x7FFF) +
							    ...
							    to_ip)
{
	...
	if ((packet = (struct packet_struct *)malloc(sizeof(*packet))) ==
	    NULL)
	{
		DEBUG(0,
		      ("create_and_init_netbios_packet: malloc fail (1) for packet struct.\n"));
		return NULL;
	}
					      ...
					      struct in_addr *register_ip)
{
	...
	if (
	    (nmb->additional =
	     (struct res_rec *)malloc(sizeof(struct res_rec))) == NULL)
	{
		DEBUG(0,
		      ("initiate_name_register_packet: malloc fail for additional record.\n"));
		return False;
	}
						       ...
						       register_ip)
{
	...
	{
		DEBUG(0,
		      ("queue_register_multihomed_name: should only be done on \
unicast subnet. subnet is %s\n.",
	...
	if (nb_flags & NB_GROUP)
		ret =
			initiate_name_register_packet(p, nb_flags,
						      &register_ip);
	else
		ret =
			initiate_multihomed_name_register_packet(p, nb_flags,
								 &register_ip);
...
static struct subnet_record *find_subnet_for_dgram_browse_packet(struct
								 packet_struct
								 *p)
{
	struct subnet_record *subrec;
...
static void process_browse_packet(struct packet_struct *p, char *buf, int len)
{
	...
	{
		DEBUG(7,
		      ("process_browse_packet: Discarding datagram from IP %s. Scope (%s) \
mismatch with our scope (%s).\n",
	...
	{
		DEBUG(0,
		      ("process_browse_packet: Discarding datagram from IP %s. Source name \
%s is one of our names !\n",
	...
	switch (command)
	{
		...
		case ANN_GetBackupListResp:
		{
			...
			DEBUG(0,
			      ("process_browse_packet: Discarding GetBackupListResponse \
packet from %s IP %s\n",
...
static void process_lanman_packet(struct packet_struct *p, char *buf, int len)
{
	...
	{
		DEBUG(7,
		      ("process_lanman_packet: Discarding datagram from IP %s. Scope (%s) \
mismatch with our scope (%s).\n",
		       inet_ntoa(p->ip), dgram->dest_name.scope, scope));
		return;
	}
	...
	{
		DEBUG(0,
		      ("process_lanman_packet: Discarding datagram from IP %s. Source name \
%s is one of our names !\n",
		       inet_ntoa(p->ip), nmb_namestr(&dgram->source_name)));
		return;
	}
	...
	switch (command)
	{
		...
		{
			DEBUG(0,
			      ("process_lanman_packet: On subnet %s ignoring browse packet \
command code %d from %s IP %s to %s\n",
			       subrec->subnet_name, command,
			       nmb_namestr(&dgram->source_name),
			       inet_ntoa(p->ip),
...
static void process_dgram(struct packet_struct *p)
{
	...
	DEBUG(4,
	      ("process_dgram: datagram from %s to %s IP %s for %s of type %d len=%d\n",
	       nmb_namestr(&dgram->source_name),
	       nmb_namestr(&dgram->dest_name), inet_ntoa(p->ip), smb_buf(buf),
	       CVAL(buf2, 0), len));
...
static BOOL validate_nmb_response_packet(struct nmb_packet *nmb)
{
	...
	switch (nmb->header.opcode)
	{
		...
		case NMB_NAME_QUERY_OPCODE:
			...
			{
				DEBUG(0,
				      ("validate_nmb_response_packet: Bad QUERY Packet. "));
				ignore = True;
			}
			break;
		case NMB_NAME_RELEASE_OPCODE:
			...
			{
				DEBUG(0,
				      ("validate_nmb_response_packet: Bad RELEASE Packet. "));
				ignore = True;
			}
			break;
		case NMB_WACK_OPCODE:
			...
			{
				DEBUG(0,
				      ("validate_nmb_response_packet: Bad WACK Packet. "));
				ignore = True;
			}
			break;
		default:
			DEBUG(0,
			      ("validate_nmb_response_packet: Ignoring packet with unknown opcode %d.\n",
			       nmb->header.opcode));
			return True;
	}
	...
	if (ignore)
		DEBUG(0,
		      ("Ignoring response packet with opcode %d.\n",
		       nmb->header.opcode));
							...
							**pprrec)
{
	...
	if (nmb->header.response)
	{
		...
		{
			DEBUG(0,
			      ("find_subnet_for_nmb_packet: response record not found for response id %hu\n",
		...
		{
			DEBUG(0,
			      ("find_subnet_for_nmb_packet: subnet record not found for response id %hu\n",
			       nmb->header.name_trn_id));
			return NULL;
		}
		...
		case NMB_NAME_QUERY_OPCODE:
			switch (nmb->question.question_type)
			{
				...
				case QUESTION_TYPE_NB_STATUS:
				{
					...
					{
						DEBUG(0,
						      ("process_nmb_request: NB_STATUS request directed at WINS server is \
not allowed.\n"));
						break;
					}
					else
				...
				int *listen_number)
{
	...
	{
		DEBUG(0,
		      ("create_listen_fdset: Too many file descriptors needed (%d). We can \
only use %d.\n",
		       (count * 2) + 2, FD_SETSIZE));
		return True;
	}

	if ((sock_array = (int *)malloc(((count * 2) + 2) * sizeof(int))) ==
	    NULL)
	{
		DEBUG(0,
		      ("create_listen_fdset: malloc fail for socket array.\n"));
		return True;
	}
		   ...
		   int dest_port)
{
	...
	DEBUG(4,
	      ("send_mailslot: Sending to mailslot %s from %s IP %s ",
