...
void winbindd_query_aliasmem(struct winbindd_cli_state *state)
{
	DOM_SID alias_sid;
	struct winbindd_domain *domain;
	...
	if (!string_to_sid(&alias_sid, state->request.data.sid)) {
		DEBUG(5, ("Could not get convert sid %s from string\n",
			  state->request.data.sid));
		request_error(state);
		return;
	}

	domain = find_domain_from_sid(&alias_sid);
...
enum winbindd_result winbindd_dual_query_aliasmem(struct winbindd_domain *domain,
						  struct winbindd_cli_state *state)
{
	DOM_SID alias_sid;
	uint32 alias_rid;
	...
	if (!sid_peek_check_rid(&domain->sid, &alias_sid, &alias_rid)) {
		DEBUG(0, ("Strange -- ALIAS SID %s does not match domain SID "
			  "%s\n", state->request.data.sid,
			  sid_string_static(&domain->sid)));
		return WINBINDD_ERROR;
	}

	status = domain->methods->query_aliasmem(domain, state->mem_ctx,
						 alias_rid, &num_members,
						 &members);

	if (!NT_STATUS_IS_OK(status)) {
		DEBUG(5, ("query_aliasmem for rid %d failed: %s\n",
			  alias_rid, nt_errstr(status)));
		return WINBINDD_ERROR;
	}
	...
	if (!print_sidlist(NULL, members, num_members, &sidstring, &len)) {
		DEBUG(0, ("malloc failed\n"));
		return WINBINDD_ERROR;
	}
	...
	state->response.extra_data = sidstring;
	state->response.length += len+1;
	state->response.data.num_entries = num_members;

	return WINBINDD_OK;
}

void winbindd_query_groupmem(struct winbindd_cli_state *state)
{
	DOM_SID group_sid;
	struct winbindd_domain *domain;
	...
	if (!string_to_sid(&group_sid, state->request.data.sid)) {
		DEBUG(5, ("Could not get convert sid %s from string\n",
			  state->request.data.sid));
		request_error(state);
		return;
...
enum winbindd_result winbindd_dual_query_groupmem(struct winbindd_domain *domain,
						  struct winbindd_cli_state *state)
{
	DOM_SID group_sid;
	uint32 group_rid;
	...
	if (!sid_peek_check_rid(&domain->sid, &group_sid, &group_rid)) {
		DEBUG(0, ("Strange -- GROUP SID %s does not match domain SID "
			  "%s\n", state->request.data.sid,
			  sid_string_static(&domain->sid)));
		return WINBINDD_ERROR;
	}

	status = domain->methods->query_groupmem(domain, state->mem_ctx,
						 group_rid, &num_members,
						 &members);

	if (!NT_STATUS_IS_OK(status)) {
		DEBUG(5, ("query_groupmem for rid %d failed: %s\n",
			  group_rid, nt_errstr(status)));
		return WINBINDD_ERROR;
	}
	...
	if (!print_ridlist(NULL, members, num_members, &ridstring, &len)) {
		DEBUG(0, ("malloc failed\n"));
		return WINBINDD_ERROR;
	}

	state->response.extra_data = ridstring;
	state->response.length += len+1;
	state->response.data.num_entries = num_members;
...
static void winbindd_groupmembers_recv(void *private_data, BOOL success,
				       uint32 num_members,
				       const char **domains,
				       const char **names);

void winbindd_group_members(struct winbindd_cli_state *state)
{
	DOM_SID group_sid;
	...
	winbindd_groupmembers_async(state->mem_ctx, &group_sid,
				    winbindd_groupmembers_recv, state);
}
				       ...
				       const char **names)
{
	...
	if (!success) {
		DEBUG(5, ("Could not get group members for SID %s\n",
			  state->request.data.sid));
		request_error(state);
		return;
	}
	...
	for (i=0; i<num_members; i++) {
		sprintf_append(NULL, &string, &len, &buflen,
			       "%s\\%s\n", domains[i], names[i]);
	}
	...
	state->response.extra_data = string;
	state->response.length += len+1;
	state->response.data.num_entries = num_members;
	request_ok(state);
}
