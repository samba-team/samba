...
enum winbindd_result winbindd_getpwnam(struct winbindd_cli_state *state) 
{
	...
	if (!winbindd_lookup_sid_by_name(state->mem_ctx, domain, domain->name, name_user, &user_sid, &name_type)) {
...
enum winbindd_result winbindd_getpwuid(struct winbindd_cli_state *state)
{
	...
	if (!winbindd_lookup_name_by_sid(state->mem_ctx, &user_sid, dom_name,
					 user_name, &name_type)) {
		fstring temp;
