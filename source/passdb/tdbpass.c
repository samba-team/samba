...
{
  /* Open tdb passwd */
  if (!(tdb_ent.passwd_tdb = tdb_open_log(lp_tdb_passwd_file(), 0, USE_TDB_MMAP_FLAG, update ? O_RDWR : O_RDONLY, 0600)))
  {
     DEBUG(0, ("Unable to open TDB passwd, trying create new!\n"));
     if (!(tdb_ent.passwd_tdb = tdb_open_log(lp_tdb_passwd_file(), 0, USE_TDB_MMAP_FLAG, O_RDWR | O_CREAT | O_EXCL, 0600)))
     {
...
static BOOL del_samtdbpwd_entry(const char *name)
{
  ...
  if (!(pwd_tdb = tdb_open_log(lp_tdb_passwd_file(), 0, USE_TDB_MMAP_FLAG, O_RDWR, 0600)))
  {
...
static struct sam_passwd *getsamtdb21pwnam(char *name)
{
  ...
  if (!(pwd_tdb = tdb_open_log(lp_tdb_passwd_file(), 0, USE_TDB_MMAP_FLAG, O_RDONLY, 0600)))
  {
     DEBUG(0, ("Unable to open TDB passwd!"));
     return False;
  }
