...
again:
	...
	len += tdb_pack(buf + len, buflen - len, NULL, "ddddddd",
...
static void print_notify_send_messages_to_printer(const char *unix_printer, unsigned int timeout)
{
	...
	for (pq = notify_queue_head; pq; pq = pq->next) {
		if (strequal_unix(unix_printer, pq->msg->printer)) {
	...
	for (pq = notify_queue_head; pq; pq = pq_next) {
		...
		if (strequal(unix_printer, pq->msg->printer)) {
...
static void send_spoolss_notify2_msg(SPOOLSS_NOTIFY_MSG *msg)
{
	...
	for (tmp_ptr = notify_queue_head; tmp_ptr; tmp_ptr = tmp_ptr->next) {
		if (tmp_ptr->msg->type == msg->type &&
				...
				strequal_unix(tmp_ptr->msg->printer, msg->printer)) {
