			     ...
			     prs_struct *ps, int depth)
{
	...
	prs_uint32("flags              ", ps, depth, &(info->flags));
	prs_uint32("pdc_status         ", ps, depth, &(info->pdc_status));
	prs_uint32("ptr_trusted_dc_name", ps, depth,
		   &(info->ptr_trusted_dc_name));
	prs_uint32("tc_status          ", ps, depth, &(info->tc_status));

	if (info->ptr_trusted_dc_name != 0)
	{
		smb_io_unistr2("unistr2", &(info->uni_trusted_dc_name),
			       info->ptr_trusted_dc_name, ps, depth);
	}
			  ...
			  prs_struct *ps, int depth)
{
	...
	if (r_l->ptr != 0)
	{
		switch (r_l->switch_value)
		{
			case 1:
			{
				net_io_netinfo_1("", &(r_l->logon.info1), ps,
						 depth);
				break;
			}
			case 2:
			{
				net_io_netinfo_2("", &(r_l->logon.info2), ps,
						 depth);
				break;
			}
			case 3:
			{
				net_io_netinfo_3("", &(r_l->logon.info3), ps,
						 depth);
				break;
			}
			default:
			{
				DEBUG(2,
				      ("net_io_r_logon_ctrl2: unsupported switch value %d\n",
				       r_l->switch_value));
				break;
			}
		}
		     ...
		     DOM_CHAL * clnt_chal)
{
	...
	make_unistr2(&(q_c->uni_logon_clnt), logon_clnt,
		     strlen(logon_clnt) + 1);
		 ...
		 uint16 sec_chan, const char *comp_name, DOM_CHAL * clnt_chal)
{
	...
	make_log_info(&(q_a->clnt_id), logon_srv, acct_name, sec_chan,
		      comp_name);
...
{
	if (q_s == NULL || cred == NULL)
		return False;

	DEBUG(5, ("make_q_srv_pwset\n"));

	make_clnt_info(&(q_s->clnt_id), logon_srv, acct_name, sec_chan,
		       comp_name, cred);
			    ...
			    int depth)
{
	...
	if (id->ptr_id_info4 != 0)
	{
		...
		smb_io_unihdr("hdr_wksta  ", &(id->hdr_wksta_name), ps,
			      depth);
		...
		smb_io_unistr2("uni_wksta_name ", &(id->uni_wksta_name),
			       id->hdr_wksta_name.buffer, ps, depth);
		smb_io_string2("str_general    ", &(id->str_general),
		   ...
		   const uchar * nt_chal_resp, int nt_chal_len)
{
	...
	if (nt_chal_resp != NULL)
	{
		...
		memcpy(nt_owf, nt_chal_resp,
		       MIN(nt_chal_len, sizeof(nt_owf)));
		nt_chal_resp = nt_owf;
	}
	if (lm_chal_resp != NULL)
	{
		/* oops.  can only send what-ever-it-is direct */
		memcpy(lm_owf, lm_chal_resp,
		       MIN(lm_chal_len, sizeof(lm_owf)));
		lm_chal_resp = lm_owf;
	}

	memcpy(id->lm_chal, lm_challenge, sizeof(id->lm_chal));
	make_str_hdr(&(id->hdr_nt_chal_resp), sizeof(nt_owf), nt_chal_len,
		     nt_chal_resp != NULL ? 1 : 0);
	make_str_hdr(&(id->hdr_lm_chal_resp), sizeof(lm_owf), lm_chal_len,
		     lm_chal_resp != NULL ? 1 : 0);
			    ...
			    int depth)
{
	...
	if (id->ptr_id_info2 != 0)
	{
		...
		smb_io_strhdr("hdr_nt_chal_resp", &(id->hdr_nt_chal_resp), ps,
			      depth);
		smb_io_strhdr("hdr_lm_chal_resp", &(id->hdr_lm_chal_resp), ps,
			      depth);
		...
		smb_io_unistr2("uni_wksta_name ", &(id->uni_wksta_name),
			       id->hdr_wksta_name.buffer, ps, depth);
		smb_io_string2("nt_chal_resp", &(id->nt_chal_resp),
			       id->hdr_nt_chal_resp.buffer, ps, depth);
		smb_io_string2("lm_chal_resp", &(id->lm_chal_resp),
			       ...
			       prs_struct *ps, int depth)
{
	...
	switch (ctr->switch_value)
	{
		...
		default:
		{
			/* PANIC! */
			DEBUG(4,
			      ("smb_io_sam_info: unknown switch_value!\n"));
			break;
		}
	}
			 ...
			 char *logon_dom, char *padding, DOM_SID *dom_sid)
{
	...
	make_unistr2(&(usr->uni_logon_script), logon_script,
		     len_logon_script);
	make_unistr2(&(usr->uni_profile_path), profile_path,
		     len_profile_path);
		      ...
		      uint16 switch_value, void *id, uint32 status)
{
	...
	if (status == NT_STATUS_NOPROBLEMO)
	{
		...
		else
		{
			...
			r_s->ctr.switch_value = 0;	/* indicates no info */
		}
	}
	else
	{
			  ...
			  prs_struct *ps, int depth)
{
	...
	prs_debug(ps, depth, desc, "net_io_user_info_ctr");
	depth++;

	prs_uint16("switch_value", ps, depth, &(ctr->switch_value));
	prs_align(ps);
	prs_uint32("ptr_user_info ", ps, depth, &ctr->ptr_user_info);

	if (ctr->switch_value != 0 && ctr->ptr_user_info != 0)
	{
		switch (ctr->switch_value)
		{
			case 2:
			...
			{
				DEBUG(0,
				      ("net_io_user_info_ctr: invalid info class %d\n",
				       ctr->switch_value));
				return False;
			}
		}
				 ...
				 prs_struct *ps, int depth)
{
	...
	switch (type)
	{
		...
		{
			DEBUG(0,
			      ("Replication error: Unknown delta type %x\n",
			       type));
			break;
		}
	}
