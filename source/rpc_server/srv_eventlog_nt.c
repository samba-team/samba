...
void eventlog_refresh_external_parameters( NT_USER_TOKEN * token )
{
	...
	for ( i = 0; elogs[i]; i++ ) {
		...
		DEBUG( 10,
		       ( "eventlog_refresh_external_parameters: Refreshing =>[%s]\n",
			 elogs[i] ) );

		if ( !control_eventlog_hook( token, elogs[i] ) ) {
			DEBUG( 0,
			       ( "eventlog_refresh_external_parameters: failed to refresh [%s]\n",
				 elogs[i] ) );
		}
	}
...
BOOL control_eventlog_hook( NT_USER_TOKEN * token, const char *elogname )
{
	...
	if ( !cmd || !*cmd ) {
		DEBUG( 0,
		       ( "control_eventlog_hook: No \"eventlog control command\" defined in smb.conf!\n" ) );
		return False;
	}
	...
	if ( !W_ERROR_IS_OK( wresult ) ) {
		DEBUG( 4,
		       ( "control_eventlog_hook: Failed to open key [%s] (%s)\n",
	...
	if ( ret != 0 ) {
		DEBUG( 10,
		       ( "control_eventlog_hook: Command returned  [%d]\n",
			 ret ) );
		if ( fd != -1 )
			close( fd );
		return False;
...
static BOOL open_eventlog_hook( EventlogInfo * info )
{
	...
	if ( !cmd || !*cmd ) {
		DEBUG( 0,
		       ( "Must define an \"eventlog open command\" entry in the config.\n" ) );
		return False;
	}
	...
	if ( numlines ) {
		...
		if ( 0 ==
		     strncmp( qlines[0], "SUCCESS", strlen( "SUCCESS" ) ) ) {
			DEBUGADD( 10,
				  ( "Able to open [%s].\n", info->logname ) );
...
static BOOL get_num_records_hook( EventlogInfo * info )
{
	...
	if ( !cmd || !*cmd ) {
		DEBUG( 0,
		       ( "Must define an \"eventlog num records command\" entry in the config.\n" ) );
		return False;
	}
...
static BOOL get_oldest_entry_hook( EventlogInfo * info )
{
	...
	if ( !cmd || !*cmd ) {
		DEBUG( 0,
		       ( "Must define an \"eventlog oldest record command\" entry in the config.\n" ) );
		return False;
	}
...
static BOOL close_eventlog_hook( EventlogInfo * info )
{
	...
	if ( !cmd || !*cmd ) {
		DEBUG( 0,
		       ( "Must define an \"eventlog close command\" entry in the config.\n" ) );
		return False;
	}
...
*/
		...
		for ( i = 0; i < entry->data_record.strings_len; i++ ) {
			fputc( ( char ) entry->data_record.strings[i],
			       stderr );
		}
		fprintf( stderr, "\nDone\n" );
				 ...
				 pstring backup_file_name )
{
	...
	if ( !cmd || !*cmd ) {
		DEBUG( 0,
		       ( "Must define an \"eventlog clear command\" entry in the config.\n" ) );
		return False;
	}
				...
				EVENTLOG_R_READ_EVENTLOG * r_u )
{
	...
	if ( !
	     ( read_eventlog_hook
	       ( info, &entry, direction, q_u->offset, q_u->max_read_size,
		 &eof, &buffer, &numlines ) ) ) {
		if ( eof == False ) {
			return WERR_NOMEM;
		}
