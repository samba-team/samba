...
static WERROR delete_printer_handle(pipes_struct *p, POLICY_HND *hnd)
{
	...
	if (*lp_deleteprinter_cmd()) {
		...
		slprintf(command, sizeof(command)-1, "%s \"%s\"", cmd,
					dos_to_unix_static(Printer->dev.handlename));
		...
		if ( ( i = lp_servicenumber_dos( Printer->dev.handlename ) ) < 0 )
			return WERR_ACCESS_DENIED;
	}
...
static BOOL get_printer_snum(pipes_struct *p, POLICY_HND *hnd, int *number)
{
	...
	case PRINTER_HANDLE_IS_PRINTER:		
		...
		*number = print_queue_snum_dos(Printer->dev.handlename);
		return (*number != -1);
...
static BOOL set_printer_hnd_name(Printer_entry *Printer, char *handlename)
{
	...
	for (snum=0; snum<n_services; snum++) {
		...
		fstrcpy(sname, lp_const_servicename_dos(snum));
...
{ 
	...
	offset += tdb_unpack((char *)buf + offset, len - offset, NULL, "ddddddd",
			     &tv_sec, &tv_usec,
...
{
	const char *printer_name = lp_const_servicename_dos(snum);
...
void do_drv_upgrade_printer(int msg_type, pid_t src, void *buf, size_t len)
{
	...
	for (snum=0; snum<n_services; snum++)
	{
		if (lp_snum_ok(snum) && lp_print_ok(snum) ) 
		{
			...
			result = get_a_printer(NULL, &printer, 2, lp_const_servicename_dos(snum));
			if (!W_ERROR_IS_OK(result))
				continue;
...
Can't find printer handle we created for printer %s\n", name ));
	...
	if (Printer->printer_type == PRINTER_HANDLE_IS_PRINTSERVER) 
	{
		...
		if ( printer_default->access_required & SERVER_ACCESS_ADMINISTER ) 
		{
			...
			if ( user.uid!=0 && !user_in_plist(uidtoname(user.uid), lp_printer_admin(snum)) ) {
				close_printer_handle(p, handle);
				return WERR_ACCESS_DENIED;
			}
...
{		
	...
	if (!strcmp(value, "MajorVersion")) {
   	...
   	if (!strcmp(value, "DefaultSpoolDirectory")) {
		...
		fstrcpy(string, string_truncate(lp_serverstring_dos(), MAX_SERVER_STRING_LENGTH));
		*type = 0x1;			
...
WERROR _spoolss_getprinterdata(pipes_struct *p, SPOOL_Q_GETPRINTERDATA *q_u, SPOOL_R_GETPRINTERDATA *r_u)
{
	...
	*type   = 0;
	...
	else
	{
		...
		status = get_a_printer(Printer, &printer, 2, lp_const_servicename_dos(snum));
		if ( !W_ERROR_IS_OK(status) )
			goto done;
...
WERROR _spoolss_rffpcnex(pipes_struct *p, SPOOL_Q_RFFPCNEX *q_u, SPOOL_R_RFFPCNEX *r_u)
{
	...
	unistr2_to_dos(Printer->notify.localmachine, localmachine, 
				      ...
				      TALLOC_CTX *mem_ctx)
{
	...
	len = (uint32)dos_PutUniCode(temp, lp_const_servicename_dos(snum), 
				     sizeof(temp) - 2, True);
				   ...
				   TALLOC_CTX *mem_ctx)
{
	...
	if (*printer->info_2->comment == '\0')
		len = (uint32)dos_PutUniCode(temp, lp_comment_dos(snum), 
					     sizeof(temp) - 2, True);
	else
				    ...
				    TALLOC_CTX *mem_ctx)
{
	...
	len = (uint32)dos_PutUniCode(temp, unix_to_dos_static(queue->fs_user), 
				     sizeof(temp) - 2, True);
				    ...
				    TALLOC_CTX *mem_ctx)
{
	...
	len = (uint32)dos_PutUniCode(temp, unix_to_dos_static(queue->fs_file), sizeof(temp) - 2, 
				     True);
					  ...
					  TALLOC_CTX *mem_ctx) 
{
	...
	DEBUG(4,("construct_notify_printer_info: Notify type: [%s], number of notify info: [%d] on printer: [%s]\n",
		...
		option_type->count, lp_const_servicename_unix(snum)));
	
	if (!W_ERROR_IS_OK(get_a_printer(print_hnd, &printer, 2, lp_const_servicename_dos(snum))))
		return False;
				  ...
				  TALLOC_CTX *mem_ctx)
{
	...
	for (i=0; i<option->count; i++) {
		...
		case JOB_NOTIFY_TYPE: {
			...
			if (!W_ERROR_IS_OK(get_a_printer(Printer, &printer, 2, lp_const_servicename_dos(snum))))
				goto done;
...
static BOOL construct_printer_info_0(Printer_entry *print_hnd, PRINTER_INFO_0 *printer, int snum)
{
	...
	if (!W_ERROR_IS_OK(get_a_printer(print_hnd, &ntprinter, 2, lp_const_servicename_dos(snum))))
		return False;
...
static BOOL construct_printer_info_1(Printer_entry *print_hnd, uint32 flags, PRINTER_INFO_1 *printer, int snum)
{
	...
	if (*ntprinter->info_2->comment == '\0') {
		init_unistr(&printer->comment, lp_comment_dos(snum));
		slprintf(chaine,sizeof(chaine)-1,"%s,%s,%s", ntprinter->info_2->printername,
			ntprinter->info_2->drivername, lp_comment_dos(snum));
	}
	else {
...
DEVICEMODE *construct_dev_mode(int snum)
{
	...
	if (!W_ERROR_IS_OK(get_a_printer(NULL, &printer, 2, lp_const_servicename_dos(snum)))) 
		return NULL;
...
static BOOL construct_printer_info_2(Printer_entry *print_hnd, PRINTER_INFO_2 *printer, int snum)
{
	...
	init_unistr(&printer->sharename, lp_const_servicename_dos(snum));			/* sharename */
	init_unistr(&printer->portname, ntprinter->info_2->portname);			/* port */	
	init_unistr(&printer->drivername, ntprinter->info_2->drivername);	/* drivername */

	if (*ntprinter->info_2->comment == '\0')
		init_unistr(&printer->comment, lp_comment_dos(snum));			/* comment */	
	else
		init_unistr(&printer->comment, ntprinter->info_2->comment); /* saved comment. */
...
static WERROR enum_all_printers_info_1(uint32 flags, NEW_BUFFER *buffer, uint32 offered, uint32 *needed, uint32 *returned)
{
	...
	for (snum=0; snum<n_services; snum++) {
		if (lp_browseable(snum) && lp_snum_ok(snum) && lp_print_ok(snum) ) {
			DEBUG(4,("Found a printer in smb.conf: %s[%x]\n", lp_const_servicename_unix(snum), snum));
...
{	
	...
	status=get_a_printer(NULL, &printer, 2, lp_const_servicename_dos(snum) );
				 ...
				 pipes_struct *p, SEC_DESC_BUF *secdesc_ctr)
{
	...
	if (DEBUGLEVEL >= 10) {
		...
		DEBUG(10, ("old_secdesc_ctr for %s has %d aces:\n", 
			   lp_printername_unix(snum), the_acl->num_aces));
                           ...
                           DEVICEMODE *devmode)
{
	...
	if (!W_ERROR_IS_OK(get_a_printer(Printer, &printer, 2, lp_const_servicename_dos(snum))) ||
	    (!W_ERROR_IS_OK(get_a_printer(Printer, &old_printer, 2, lp_const_servicename_dos(snum))))) {
		result = WERR_BADFID;
		goto done;
	}
	...
	init_unistr2( &buffer, global_myname_unix(), strlen(global_myname_unix())+1 );
	...
	slprintf( asc_buffer, sizeof(asc_buffer)-1, "\\\\%s\\%s",
                 global_myname_unix(), printer->info_2->sharename );
                            ...
                            int position, int snum)
{
	...
	init_unistr(&job_info->printername, lp_const_servicename_dos(snum));
	init_unistr(&job_info->machinename, temp_name);
	init_unistr(&job_info->username, unix_to_dos_static(queue->fs_user));
	init_unistr(&job_info->document, unix_to_dos_static(queue->fs_file));
			    ...
			    DEVICEMODE *devmode)
{
	...
	init_unistr(&job_info->username, unix_to_dos_static(queue->fs_user));
	init_unistr(&job_info->document, unix_to_dos_static(queue->fs_file));
	init_unistr(&job_info->notifyname, unix_to_dos_static(queue->fs_user));
	init_unistr(&job_info->datatype, "RAW");
			      ...
			      uint32 *needed, uint32 *returned)
{
	...
	result = get_a_printer(NULL, &ntprinter, 2, lp_const_servicename_dos(snum));
	if (!W_ERROR_IS_OK(result)) {
		*returned = 0;
		goto done;
	}
...
static WERROR enumports_level_2(NEW_BUFFER *buffer, uint32 offered, uint32 *needed, uint32 *returned)
{
	...
	if (*lp_enumports_cmd()) {
		...
		if (*lp_pathname_dos(lp_servicenumber_dos(PRINTERS_NAME)))
			path = lp_pathname_dos(lp_servicenumber_dos(PRINTERS_NAME));
		else
			path = lp_lockdir();
				...
				POLICY_HND *handle)
{
	...
	if ((snum = print_queue_snum_dos(printer->info_2->sharename)) != -1) {
...
WERROR _spoolss_enumprinterdata(pipes_struct *p, SPOOL_Q_ENUMPRINTERDATA *q_u, SPOOL_R_ENUMPRINTERDATA *r_u)
{
	...
	*out_data_len     = 0;
	...
	result = get_a_printer(Printer, &printer, 2, lp_const_servicename_dos(snum));
	if (!W_ERROR_IS_OK(result))
		return result;
	...
	else 
	{
		...
		*out_value_len = (uint32)dos_PutUniCode((char *)*out_value, regval_name(val), in_value_len, True);
...
WERROR _spoolss_setprinterdata( pipes_struct *p, SPOOL_Q_SETPRINTERDATA *q_u, SPOOL_R_SETPRINTERDATA *r_u)
{
	...
	unistr2_to_dos( valuename, value, sizeof(valuename)-1 );
...
static WERROR getjob_level_2(print_queue_struct *queue, int count, int snum, uint32 jobid, NEW_BUFFER *buffer, uint32 offered, uint32 *needed)
{
	...
	ret = get_a_printer(NULL, &ntprinter, 2, lp_const_servicename_dos(snum));
...
WERROR _spoolss_getprinterdataex(pipes_struct *p, SPOOL_Q_GETPRINTERDATAEX *q_u, SPOOL_R_GETPRINTERDATAEX *r_u)
{
	...
	DEBUG(4,("_spoolss_getprinterdataex\n"));
        ...
        unistr2_to_dos(keyname, &q_u->keyname, sizeof(keyname) - 1);
        unistr2_to_dos(valuename, &q_u->valuename, sizeof(valuename) - 1);
...
WERROR _spoolss_deleteprinterdataex(pipes_struct *p, SPOOL_Q_DELETEPRINTERDATAEX *q_u, SPOOL_R_DELETEPRINTERDATAEX *r_u)
{
	...
	unistr2_to_dos( valuename, value, sizeof(valuename)-1 );
	unistr2_to_dos( keyname, key, sizeof(keyname)-1 );
