...
static int dochild(int master, char *slavedev, const char *_name,
...
static BOOL chat_with_program(char *passwordprogram, const char *name,
			      char *chatsequence, BOOL as_root)
{
	...
	{
		DEBUG(3,
		      ("Cannot sys_fork() child for password change: %s\n",
		       name));
		close(master);
		CatchChild();
		return (False);
			       ...
			       UNISTR2 *new_passwd)
{
	...
	if (!nt_pass_set)
	{
		...
		nt_lm_owf_gen(buf, new_ntp16, new_p16);

		make_unistr2(new_passwd, buf, strlen(buf) + 1);
	...
	else
	{
		...
		for (i = 0; i < new_passwd->uni_str_len; i++)
		{
			new_passwd->buffer[i] = SVAL(buf, i * 2);
		}
		     ...
		     const uchar * ntdata, const uchar * nthash)
{
		    ...
		    IS_BITS_SET_ALL(sampw->acct_ctrl, ACB_PWLOCK)))
	{
		...
		{
			DEBUG(3,
			      ("pass_oem_change: account %s disabled (%x)\n",
