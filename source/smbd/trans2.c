			      ...
			      char **pparams, char **ppdata)
{
  ...
  switch (info_level) 
  {
    ...
    case SMB_QUERY_FS_ATTRIBUTE_INFO:
      ...
      PutUniCode(pdata+12,unix_to_dos(fstype,False));
    ...
    case SMB_QUERY_FS_VOLUME_INFO:      
      ...
      if (global_client_caps & CAP_NT_SMBS) {
	      ...
	      PutUniCode(pdata+18,unix_to_dos(vname,False));      
      }
  ...
  switch (info_level) 
    ...
    case SMB_QUERY_FILE_ALT_NAME_INFO:
      {
        ...
        strupper(short_name);
        l = strlen(short_name);
        PutUniCode(pdata + 4, unix_to_dos(short_name,False));
        data_size = 4 + (2*l);
        SIVAL(pdata,0,2*l);
      }
      break;
    ...
    case SMB_QUERY_FILE_NAME_INFO:
      ...
      if(strequal(".", fname) && (global_client_caps & CAP_UNICODE)) {
        l = l*2;
        SSVAL(outbuf,smb_flg2,SVAL(outbuf,smb_flg2)|FLAGS2_UNICODE_STRINGS);
        PutUniCode(pdata + 4, unix_to_dos("\\",False));
      } else {
