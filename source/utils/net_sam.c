...
static int net_sam_rights_grant(int argc, const char **argv)
{
	...
	if (!se_priv_from_name(argv[1], &mask)) {
		d_fprintf(stderr, "Could not find right %s\n", argv[1]);
		return -1;
	}

	if (!grant_privilege(&sid, &mask)) {
		d_fprintf(stderr, "Adding right failed\n");
		return -1;
	}

	d_printf("Gave %s\\%s the right %s\n", dom, name, argv[1]);
	return 0;
}
...
static int net_sam_rights_revoke(int argc, const char **argv)
{
	...
	d_printf("Revoked the right %s from %s\\%s\n", argv[1], dom, name);
	return 0;
}

/*
 * Show the rights assigned to an account or vice versa
 */

static int net_sam_rights_show(int argc, const char **argv)
{
	DOM_SID sid;
	const char *dom, *name;
	SE_PRIV mask;
	...
	if (argc != 1) {
		d_fprintf(stderr, "usage: net sam rights show "
			  "[right|name]");
		return -1;
	}
	...
	if (se_priv_from_name(argv[0], &mask)) {
		...
		privilege_enumerate_privileged(tmp_talloc_ctx(), &mask,
					       &sids, &num_sids);
		for (i=0; i<num_sids; i++) {
			if (lookup_sid(tmp_talloc_ctx(), &sids[i],
				       &dom, &name, NULL)) {
				d_printf("%s\\%s\n", dom, name);
			} else {
	...
	if (!lookup_name(tmp_talloc_ctx(), argv[0], LOOKUP_NAME_ISOLATED,
			 &dom, &name, &sid, NULL)) {
		d_fprintf(stderr, "%s is neither a right name nor an "
			  "account name.\n", argv[0]);
		return -1;
	}

	/* Ignore the error code, nothing in the tdb is the same as no rights
	 * assigned */
	get_privileges_for_sids(&mask, &sid, 1);

	count = count_all_privileges();
...
static int net_sam_rights_list(int argc, const char **argv)
{
	...
	if (argc != 0) {
		d_fprintf(stderr, "usage: net sam rights list\n");
		return -1;
	}
...
/*
 * List, grant and revoke privileges
 */
...
static int net_sam_rights(int argc, const char **argv)
{
	struct functable2 func[] = {
		...
		{ "grant", net_sam_rights_grant,
		  "Grant a right to an account" },
		{ "revoke", net_sam_rights_revoke,
		  "Revoke a right from account" },
		{ "show", net_sam_rights_show,
		  "Show right assignments" },
		{ NULL, NULL, NULL }
	};

	if (!lp_enable_privileges()) {
		d_fprintf(stderr, "To use privileges, set "
