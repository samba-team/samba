						 ...
						 const char **_ccache_name)
{
	...
	creds = gensec_get_credentials(gensec);
	if (creds == NULL) {
		TALLOC_FREE(frame);
	...
	status = kerberos_prepare_cli_credentials_ccache(creds,
							 NULL, /* explicit_kdc */
							 debug_target);
	if (!NT_STATUS_IS_OK(status)) {
		TALLOC_FREE(frame);
	...
	ok = cli_credentials_get_ccache_name_obtained(creds,
						      gensec,
						      &ccache_name,
						      NULL); /* obtained */
	if (!ok) {
