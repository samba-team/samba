			       ...
			       uint32_t attrs)
{
	...
	status = SMB_VFS_CREATE_FILE(conn,	    /* conn */
				     req,	    /* req */
				     dst_dirfsp,    /* dirfsp */
				     smb_fname_dst, /* fname */
	...
	/* Grrr. We have to do this as open_file_ntcreate adds
	   FILE_ATTRIBUTE_ARCHIVE when it creates the file. This isn't the
	   correct thing to do in the copy case. JRA */
	...
	if (smb_fname_dst->fsp == NULL) {
		...
		/* should we handle NT_STATUS_OBJECT_NAME_NOT_FOUND specially
		 * here ???? */
		if (!NT_STATUS_IS_OK(status)) {
			goto out;
		}
