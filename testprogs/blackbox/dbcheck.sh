...
{
	$PYTHON $BINDIR/samba-tool dbcheck --quiet --fix --yes-restricted-to-rule=fix_all_old_dn_string_component_mismatch --attrs="lastKnownParent defaultObjectCategory fromServer rIDSetReferences" --cross-ncs $ARGS
}

# This list of attributes can be freely extended
dbcheck_fix_stale_links()
{
	$PYTHON $BINDIR/samba-tool dbcheck --quiet --fix --yes-restricted-to-rule=remove_plausible_deleted_DN_links --attrs="member msDS-NC-Replica-Locations msDS-NC-RO-Replica-Locations msDS-RevealOnDemandGroup msDS-NeverRevealGroup msDS-RevealedUsers" --cross-ncs $ARGS
}
...
dbcheck_fix_crosspartition_backlinks()
{
	...
	$PYTHON $BINDIR/samba-tool dbcheck --quiet --fix --yes-restricted-to-rule=fix_all_missing_backlinks --attrs="serverReference" --cross-ncs $ARGS
}
