...
tombstones_expunge()
{
	tmpfile=$PREFIX/$RELEASE/expected-expunge-output.txt.tmp
	tmpldif1=$PREFIX/$RELEASE/expected-expunge-output2.txt.tmp1

	TZ=UTC $ldbsearch -H tdb://$PREFIX/${RELEASE}/private/sam.ldb --scope=base -b '' | grep highestCommittedUSN >$tmpldif1

	$PYTHON $BINDIR/samba-tool domain tombstones expunge -H tdb://$PREFIX/${RELEASE}/private/sam.ldb --current-time=2016-07-30 --tombstone-lifetime=4 >$tmpfile
	if [ "$?" != "0" ]; then
		return 1
	fi
	...
	tmpldif2=$PREFIX/$RELEASE/expected-expunge-output2.txt.tmp2
...
add_unsorted_links()
{
	...
	TZ=UTC $ldbmodify -H tdb://$PREFIX/${RELEASE}/private/sam.ldb $ldif --relax
	if [ "$?" != "0" ]; then
		return 1
	fi
...
{
	tmpldif=$PREFIX/$RELEASE/expected-match-rule-links.ldif.tmp
	TZ=UTC $ldbsearch -H tdb://$PREFIX/${RELEASE}/private/sam.ldb '(member:1.3.6.1.4.1.7165.4.5.2:=131139216000000000)' --scope=sub -b DC=release-4-5-0-pre1,DC=samba,DC=corp --show-deleted --reveal --sorted no_attrs >$tmpldif
...
{
	$ldbsearch -H tdb://$PREFIX/${RELEASE}/private/sam.ldb '(member:1.3.6.1.4.1.7165.4.5.2:=-131139216000000000)' --scope=sub -b DC=release-4-5-0-pre1,DC=samba,DC=corp --show-deleted --reveal --sorted member
}

check_match_rule_links_overflow()
{
	$ldbsearch -H tdb://$PREFIX/${RELEASE}/private/sam.ldb '(member:1.3.6.1.4.1.7165.4.5.2:=18446744073709551617)' --scope=sub -b DC=release-4-5-0-pre1,DC=samba,DC=corp --show-deleted --reveal --sorted member
}

check_match_rule_links_null()
{
	$ldbsearch -H tdb://$PREFIX/${RELEASE}/private/sam.ldb '(member:1.3.6.1.4.1.7165.4.5.2:=18446744\073709551617)' --scope=sub -b DC=release-4-5-0-pre1,DC=samba,DC=corp --show-deleted --reveal --sorted member
}

check_match_rule_links_hex()
{
	$ldbsearch -H tdb://$PREFIX/${RELEASE}/private/sam.ldb '(member:1.3.6.1.4.1.7165.4.5.2:=abcd)' --scope=sub -b DC=release-4-5-0-pre1,DC=samba,DC=corp --show-deleted --reveal --sorted member
}

check_match_rule_links_hex2()
{
	$ldbsearch -H tdb://$PREFIX/${RELEASE}/private/sam.ldb '(member:1.3.6.1.4.1.7165.4.5.2:=0xabcd)' --scope=sub -b DC=release-4-5-0-pre1,DC=samba,DC=corp --show-deleted --reveal --sorted member
}

check_match_rule_links_decimal()
{
	$ldbsearch -H tdb://$PREFIX/${RELEASE}/private/sam.ldb '(member:1.3.6.1.4.1.7165.4.5.2:=131139216000000000.00)' --scope=sub -b DC=release-4-5-0-pre1,DC=samba,DC=corp --show-deleted --reveal --sorted member
}

check_match_rule_links_backlink()
{
	$ldbsearch -H tdb://$PREFIX/${RELEASE}/private/sam.ldb '(memberOf:1.3.6.1.4.1.7165.4.5.2:=131139216000000000)' --scope=sub -b DC=release-4-5-0-pre1,DC=samba,DC=corp --show-deleted --reveal --sorted memberOf
}

check_match_rule_links_notlink()
{
	$ldbsearch -H tdb://$PREFIX/${RELEASE}/private/sam.ldb '(samAccountName:1.3.6.1.4.1.7165.4.5.2:=131139216000000000)' --scope=sub -b DC=release-4-5-0-pre1,DC=samba,DC=corp --show-deleted --reveal --sorted samAccountName
}

check_expected_after_links()
{
	tmpldif=$PREFIX/$RELEASE/expected-links-after-expunge.ldif.tmp
...
{
	tmpldif=$PREFIX/$RELEASE/expected-deleted-links-after-expunge.ldif.tmp
...
{
	tmpldif=$PREFIX/$RELEASE/expected-objects-after-expunge.ldif.tmp
	TZ=UTC $ldbsearch -H tdb://$PREFIX/${RELEASE}/private/sam.ldb '(|(samaccountname=fred)(samaccountname=ddg)(samaccountname=usg)(samaccountname=user1)(samaccountname=user2))' --scope=sub -b DC=release-4-5-0-pre1,DC=samba,DC=corp --show-deleted --reveal --sorted samAccountName | grep sAMAccountName >$tmpldif
...
{
	tmpldif=$PREFIX/$RELEASE/expected-unsorted-links-after-expunge.ldif.tmp
	TZ=UTC $ldbsearch -H tdb://$PREFIX/${RELEASE}/private/sam.ldb '(name=unsorted-g)' --scope=sub -b DC=release-4-5-0-pre1,DC=samba,DC=corp --show-deleted --reveal --sorted member >$tmpldif
