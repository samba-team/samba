...
DATA_BLOB *data_blob_talloc_p(TALLOC_CTX *mem_ctx, const void *p,
			     size_t length)
{
	DATA_BLOB *result;

	result = TALLOC_P(mem_ctx, DATA_BLOB);
	if (result == NULL) {
		DEBUG(0, ("talloc failed\n"));
		return NULL;
	}

	*result = data_blob_talloc(result, p, length);
	return result;
}
