...
static BOOL prepare_unix_socket(int sock, const char *socket_dir,
				const char *socket_name, mode_t dir_perms)
{
		     ...
		     mode_t dir_perms)
{
	...
	if (!prepare_unix_socket(sock, socket_dir, socket_name, dir_perms)) {
		close(sock);
		return -1;
	}
        
	if (listen(sock, 5) == -1) {
		DEBUG(0, ("listen failed on pipe socket %s: %s/%s\n", 
			  socket_dir, socket_name, strerror(errno)));
		close(sock);
		return -1;
	}
...
int create_dgram_sock(const char *socket_dir,
		      const char *socket_name,
		      mode_t dir_perms)
{
#ifdef HAVE_UNIXSOCKET
